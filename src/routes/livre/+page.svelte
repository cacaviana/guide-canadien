<script lang="ts">
  import { bookChapters } from '$lib/book';
...
<svelte:head>
  <title>Livre complet - Decouvrir le Canada</title>
</svelte:head>

<div class="hero">
  <div class="container">
    <h1>Livre complet « Découvrir le Canada »</h1>
    <p>Texte intégral du guide officiel, chapitre par chapitre, avec audio FR et PT-BR à télécharger</p>
  </div>
</div>

<div class="container study-layout">
  <nav class="chapter-nav">
    {#each bookChapters as chapter, i}
      <button
        class="chapter-btn"
      ...
      <div class="lang-grid">
        <article class="lang-card lang-fr">
          <header class="lang-header">
            ...
            <h2>{bookChapters[activeChapter].title}</h2>
          </header>

          <div class="audio-player">
            <audio controls preload="none" src={bookChapters[activeChapter].audioFr}>
              Votre navigateur ne supporte pas l'audio.
            </audio>
            <div class="audio-meta">
              <small>Voix: Antoine (FR-CA)</small>
              <a class="download-btn" href={bookChapters[activeChapter].audioFr} download={`${bookChapters[activeChapter].id}-fr.mp3`} title="Telecharger l'audio">
          ...
          <div class="text-content">
            {#each bookChapters[activeChapter].textFr.split('\n\n') as paragraph}
              <p>{paragraph}</p>
            {/each}
          </div>
        </article>
        ...
        <article class="lang-card lang-br">
          <header class="lang-header">
            ...
            <h2>{bookChapters[activeChapter].titleBr}</h2>
          </header>

          <div class="audio-player">
            <audio controls preload="none" src={bookChapters[activeChapter].audioBr}>
              Seu navegador nao suporta audio.
            </audio>
            <div class="audio-meta">
              <small>Voz: Antônio (PT-BR)</small>
              <a class="download-btn" href={bookChapters[activeChapter].audioBr} download={`${bookChapters[activeChapter].id}-br.mp3`} title="Baixar audio">
                <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
                  <polyline points="7 10 12 15 17 10"/>
                  <line x1="12" y1="15" x2="12" y2="3"/>
          ...
          <div class="text-content">
            {#each bookChapters[activeChapter].textBr.split('\n\n') as paragraph}
              <p>{paragraph}</p>
            {/each}
          </div>
        </article>
      ...
      <div class="nav-actions">
        {#if activeChapter > 0}
          <button class="nav-btn" onclick={() => changeChapter(activeChapter - 1)}>
            &larr; {bookChapters[activeChapter - 1].title}
          </button>
        {:else}
          <span></span>
        {/if}
        {#if activeChapter < bookChapters.length - 1}
          <button class="nav-btn" onclick={() => changeChapter(activeChapter + 1)}>
            {bookChapters[activeChapter + 1].title} &rarr;
          </button>
        {/if}
      </div>
    </section>
...
<style>
  ...
  .hero h1 { font-size: 1.3rem; font-weight: 800; margin-bottom: 6px; padding: 0 12px; }
  ...
  @media (min-width: 768px) {
    ...
    .text-content { padding: 20px; max-height: 65vh; }
